...
| Telemetry                          | Array      | Array of strings for events that should be logged. If not specified, defaults to `Everything` (unless `EnableForkAndExitLogging` is `false` in which case `Fork` and `Exit` will not be logged).<br />Allowed values: `Everything`, `Execution`, `Fork`, `Exit`, `Close`, `Rename`, `Unlink`, `Link`, `ExchangeData`, `Disk`, `Bundle`, `Allowlist`, `FileAccess`, `CodesigningInvalidated`, `LoginWindowSession`, `LoginLogout`, `ScreenSharing`, `OpenSSH`, `Authentication`, `None` |
