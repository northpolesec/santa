...
- (void)handleAPNSMessage:(NSDictionary *)message {
  ...
  // 0 then the sync should happen immediately. If the value is 1 then the sync should happen after a
  // random delay between now and the global rule sync deadline. This allows sync servers implementing
  // APNS support to smear syncs of a large number of hosts to avoid a thundering herd, while still
  // reaching resolution within a reasonable time period.
