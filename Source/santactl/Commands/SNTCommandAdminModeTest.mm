...
// Defined in SNTCommandAdminMode.mm. Declared here rather than in a header so
// the command's internals stay private, matching SNTCommandDoctorTest.mm.
extern NSNumber* SNTAdminModeDurationMinutes(NSString* arg, NSError** error);

@interface SNTCommandAdminModeTest : XCTestCase
@end

@implementation SNTCommandAdminModeTest

- (void)assertMinutes:(NSString*)input equals:(int64_t)expected {
  NSError* err = nil;
  NSNumber* got = SNTAdminModeDurationMinutes(input, &err);
  XCTAssertNotNil(got, @"\"%@\" was rejected: %@", input, err.localizedDescription);
  XCTAssertEqual(got.longLongValue, expected, @"\"%@\"", input);
  // Integer-backed, never floating point: the daemon reads this NSNumber through
  // two different unsigned accessors, and those disagree for a double carrier.
...
- (void)assertRejected:(NSString*)input messageContains:(NSString*)needle {
  NSError* err = nil;
  XCTAssertNil(SNTAdminModeDurationMinutes(input, &err), @"\"%@\" was accepted", input);
...
// Covers only what this command adds on top of the shared parser, which
// SNTCommandTest exercises directly: the minutes default unit, the conversion
// to minutes, and the two constraints the parser deliberately leaves to callers.
- (void)testDurationArgumentResolvesToWholeMinutes {
  ...
  // Rejected before it can become an NSNumber the daemon reads inconsistently
  // through its two unsigned accessors, which would slip past the policy cap.
