...
- (void)testParseFileAbsolutePath {
  auto result = ParseSandboxExecArgv(
      {"sandbox-exec", "-f", "/etc/sandbox/policy.sb", "/usr/bin/ls"});
...
- (void)testParseFileRelativePath {
  auto result = ParseSandboxExecArgv(
      {"sandbox-exec", "-f", "./block-tmp-reads.sb", "/usr/bin/claude"});
  XCTAssertTrue(result.has_value());
  XCTAssertEqual(result->profile_path, "./block-tmp-reads.sb");
  XCTAssertEqual(result->status, SandboxPolicyStatus::kPending);
}

- (void)testParseInlineProfile {
  auto result = ParseSandboxExecArgv(
      {"sandbox-exec", "-p", "(version 1)(deny default)", "/usr/bin/ls"});
  XCTAssertTrue(result.has_value());
  XCTAssertEqual(result->profile_path, "");
  XCTAssertEqual(result->status, SandboxPolicyStatus::kPending);
}
...
- (void)testParseFileWithoutExtension {
  auto result = ParseSandboxExecArgv(
      {"sandbox-exec", "-f", "/etc/sandbox/policy", "/usr/bin/ls"});
  XCTAssertTrue(result.has_value());
...
- (void)testPendingConfirmedOnNextExec {
  ...
  self.tree->HandleExec(event_id++, *forked, sandbox_exec_pid,
                         sandbox_exec_prog, cred);

  // Verify pending annotation on sandbox-exec process.
  auto sandbox_proc = *self.tree->Get(sandbox_exec_pid);
  auto pending_opt =
      self.tree->GetAnnotation<SandboxExecAnnotator>(*sandbox_proc);
  ...
  self.tree->HandleExec(event_id++, *sandbox_proc, target_pid, target_prog,
                         cred);

  // Verify confirmed annotation on target process.
  auto target_proc = *self.tree->Get(target_pid);
  auto confirmed_opt =
      self.tree->GetAnnotation<SandboxExecAnnotator>(*target_proc);
  XCTAssertTrue(confirmed_opt.has_value());
  XCTAssertTrue((*confirmed_opt)->info().has_value());
  XCTAssertEqual((*confirmed_opt)->info()->status,
                 SandboxPolicyStatus::kConfirmed);
  ...
  XCTAssertEqual(proto_opt->sandbox_policy().profile_path(),
                 "/path/to/policy.sb");
}
...
- (void)testAnnotationPropagatesForkToChildren {
  ...
  auto child_opt =
      self.tree->GetAnnotation<SandboxExecAnnotator>(*child_proc);
  XCTAssertTrue(child_opt.has_value());
  XCTAssertTrue((*child_opt)->info().has_value());
  XCTAssertEqual((*child_opt)->info()->status,
                 SandboxPolicyStatus::kConfirmed);
}
...
- (void)testAnnotationPropagatesExecToChildren {
  ...
  self.tree->HandleExec(event_id++, *child_fork, child_exec_pid, child_prog,
                         cred);

  // The exec'd child should still have the annotation.
  auto child_exec = *self.tree->Get(child_exec_pid);
  auto child_opt =
      self.tree->GetAnnotation<SandboxExecAnnotator>(*child_exec);
  XCTAssertTrue(child_opt.has_value());
  XCTAssertEqual((*child_opt)->info()->status,
                 SandboxPolicyStatus::kConfirmed);
}
...
- (void)testPidReuseDoesNotMisattribute {
  ...
  auto annotation =
      self.tree->GetAnnotation<SandboxExecAnnotator>(*new_proc);
  XCTAssertFalse(annotation.has_value());
}

// Test: inline profile with -p flag.
- (void)testInlineProfile {
  uint64_t event_id = 1;
  ...
  const struct Program sandbox_exec_prog = {
      .executable = "/usr/bin/sandbox-exec",
      .arguments = {"sandbox-exec", "-p", "(version 1)(deny default)",
                    "/usr/bin/ls"},
      .code_signing = kSandboxExecCS,
  };
  auto forked = *self.tree->Get(fork_pid);
  ...
  auto confirmed_opt =
      self.tree->GetAnnotation<SandboxExecAnnotator>(*target_proc);
  XCTAssertTrue(confirmed_opt.has_value());
  XCTAssertEqual((*confirmed_opt)->info()->profile_path, "");
}
...
- (void)testExportAnnotations {
  ...
  const struct Program sandbox_exec_prog = {
      .executable = "/usr/bin/sandbox-exec",
      .arguments = {"sandbox-exec", "-f", "/etc/sandbox/agent.sb",
                    "/usr/bin/agent"},
      .code_signing = kSandboxExecCS,
  };
  auto forked = *self.tree->Get(fork_pid);
  ...
  XCTAssertEqual(exported->sandbox_policy().profile_path(),
                 "/etc/sandbox/agent.sb");
}
