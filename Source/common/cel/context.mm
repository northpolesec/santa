...
absl::optional<cel_runtime::CelValue> SantaActivation::FindValue(
  ...
  for (int i = 1; i < retDescriptor->value_count(); i++) {
    if (name == retDescriptor->value(i)->name()) {
      return cel_runtime::CelValue::CreateInt64(retDescriptor->value(i)->number());
    }
  }
...
std::vector<std::pair<absl::string_view, ::cel::Type>> SantaActivation::GetVariables(
  ...
  auto ctxDescriptor = pbv1::Context::descriptor();
...
cel_runtime::CelValue SantaActivation::CELValueFromVector(const std::vector<std::string> &v,
                                                          google::protobuf::Arena *arena) {
